...
lin Line_Item_NP__Conj item np conj = item ++ np ++ ";" ++ conj ;
lin Line_Item_S_ item s = item ++ s ++ "." ;
lin Line_Item_S__Conj item s conj = item ++ s ++ ";" ++ conj ;
lin Line_Item_S_if_NP_ item s np = item ++ s ++ "if" ++ np ++ "—" ;
lin Line_Item_Time__VP__Conj item time vp conj = item ++ time ++ "," ++ vp ++ ";" ++ conj ;
lin Line_Item_VP_c item vp = item ++ vp ++ "," ;
lin Line_Item_VP_p item vp = item ++ vp ++ "." ;
lin Line_Item_VP__Conj item vp conj = item ++ vp ++ ";" ++ conj ;
lin Line_Item_an_CN_is_not__PP__to_be_regarded_as_NP_of_ item cn pp np = item ++ "an" ++ cn ++ "is" ++ "not" ++ "," ++ pp ++ "," ++ "to" ++ "be" ++ "regarded" ++ "as" ++ np ++ "of" ++ "—" ;
lin Line_Item_an_CN_must_not_VP_if_ item cn vp = item ++ "an" ++ cn ++ "must" ++ "not" ++ vp ++ "if" ++ "—" ;
lin Line_Item_any_CN_ item cn = item ++ "any" ++ cn ++ "." ;
lin Line_Item_any_CN__Conj item cn conj = item ++ "any" ++ cn ++ ";" ++ conj ;
lin Line_Item_PP__a_CN_is_deemed_not_to_be_a_CN_ item pp cn cn2 = item ++ pp ++ "," ++ "a" ++ cn ++ "is" ++ "deemed" ++ "not" ++ "to" ++ "be" ++ "a" ++ cn2 ++ "." ;
lin Line_Item_if_S__Conj item s conj = item ++ "if" ++ s ++ ";" ++ conj ;
...
lin Line_Item_PP__where_S__S_ item pp s s2 = item ++ pp ++ "," ++ "where" ++ s ++ "," ++ s2 ++ "." ;
lin Line_Item_PP__Time__S_ item pp time s = item ++ pp ++ "," ++ time ++ "," ++ s ++ "." ;
lin Line_Item_where_a_CN_VP_ item cn vp = item ++ "where" ++ "a" ++ cn ++ vp ++ "—" ;
lin Line_Item_PP__a_CN_is_deemed_to_VP_ item pp cn vp = item ++ pp ++ "," ++ "a" ++ cn ++ "is" ++ "deemed" ++ "to" ++ vp ++ "—" ;
lin Line_QCN__in_relation_to_NP__means_ qcn np = qcn ++ "," ++ "in" ++ "relation" ++ "to" ++ np ++ "," ++ "means" ++ "—" ;
lin Line_QCN_means_any_CN_ qcn cn = qcn ++ "means" ++ "any" ++ cn ++ ";" ;
lin Line_Ref ref = ref ;
lin Line_Ref__PP__unless_S_ ref loc s = ref ++ "." ++ loc ++ "," ++ "unless" ++ s ++ "—" ;
lin Line_Ref__where_an_CN_ ref cn = ref ++ "." ++ "where" ++ "an" ++ cn ++ "—" ;
lin Line_Ref___Item_a_CN_is_a_CN_if_NP_ ref item cn cn2 np = ref ++ "." ++ "—" ++ item ++ "a" ++ cn ++ "is" ++ "a" ++ cn2 ++ "if" ++ np ++ "—" ;
lin Line_Ref___Item_this_section_applies_to_a_CN_ ref item cn = ref ++ "." ++ "—" ++ item ++ "this" ++ "section" ++ "applies" ++ "to" ++ "a" ++ cn ++ "." ;
lin Line_Ref___Item_where_S__S_ ref item s s2 = ref ++ "." ++ "—" ++ item ++ "where" ++ s ++ "," ++ s2 ++ "." ;
lin Line_S_ s = s ++ "." ;
...
lin QCN_affected_individual = "“affected" ++ "individual”" ;
lin QCN_data_breach = "“data" ++ "breach”" ;
