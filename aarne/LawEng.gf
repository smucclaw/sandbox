...
lin 'Line_Item_PP_,_a_CN_is_deemed_not_to_be_a_CN_.' item pp cn cn2 = item ++ pp ++ "," ++ "a" ++ cn ++ "is" ++ "deemed" ++ "not" ++ "to" ++ "be" ++ "a" ++ cn2 ++ "." ;
...
lin 'Line_Item_PP_,_where_S_,_S_.' item pp s s2 = item ++ pp ++ "," ++ "where" ++ s ++ "," ++ s2 ++ "." ;
lin 'Line_Item_PP_,_Time_,_S_.' item pp time s = item ++ pp ++ "," ++ time ++ "," ++ s ++ "." ;
lin 'Line_Item_where_a_CN_VP_—' item cn vp = item ++ "where" ++ "a" ++ cn ++ vp ++ "—" ;
lin 'Line_Item_PP_,_a_CN_is_deemed_to_VP_—' item pp cn vp = item ++ pp ++ "," ++ "a" ++ cn ++ "is" ++ "deemed" ++ "to" ++ vp ++ "—" ;
...
lin 'Line_Ref_._PP_,_unless_S_—' ref loc s = ref ++ "." ++ loc ++ "," ++ "unless" ++ s ++ "—" ;
...
---- lin Ref_ConjItem conjitem = conjitem ;
---- lin Ref_Item item = item ;
