def Quality_Change_logic ( Legendary, Conjured, Aged_Brie, Backstage_Pass, Sell_In, Quality ) :
  ...
  elif (Legendary == False and Conjured == False and Aged_Brie == False and Backstage_Pass == True): # 6
    return {"Delta Sell In":"sellIn  -1", "Delta Quality":"max(0 quality +1)"};
    # Backstage Pass (Fresh)

  elif (Legendary == False and Conjured == False and Aged_Brie == False and Backstage_Pass == True and (6.0 <= Sell_In and Sell_In <= 10.0)): # 7
    return {"Delta Sell In":"sellIn  -1", "Delta Quality":"max(0 quality +2)"};
    # Backstage Pass (<= 10 days to concert)

  elif (Legendary == False and Conjured == False and Aged_Brie == False and Backstage_Pass == True and (1.0 <= Sell_In and Sell_In <= 5.0)): # 8
    return {"Delta Sell In":"sellIn  -1", "Delta Quality":"max(0 quality +3)"};
    # Backstage Pass (<= 5 days to concert)

  elif (Legendary == False and Conjured == False and Aged_Brie == False and Backstage_Pass == True and Sell_In < 0.0): # 9
