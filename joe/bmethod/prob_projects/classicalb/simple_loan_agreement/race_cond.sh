...
# non deadlocking.
# By non-Zeno, we mean weakly wrt the discrete tic transition.
# Further assume that:
...
# - we assumed the deontic in the notify borrower of default rule to be a
...
# LTL model checking: memory usage approx. 204809 KiB, 5431459 atoms and 1525206 transitions generated
# LTL model checking: total time 106598ms, 83320 callbacks needed 103600ms, netto 2998ms.
# LTL model checking (fairness): 0 strongly connected components were rejected.
# LTL model checking (fairness): total fairness checking time 0ms, 0 callbacks needed 0ms, netto 0ms.
# LTL Formula TRUE.
