...
\info{
  ...
  Most of this was originally written based on the work done in mcrl2 but is
  now being rewritten for L4 and its connection to the modelling done in B.
}
...
We argue that another weakness of the approach is that aside from complexity, it
is not natural to think of a contract in terms of a DFA and encode it as such.
...
\inlinetodo{
  ...
  constructs and abstractions corresponding to concepts found in the application
  domain.
...
\inlinetodo{
  ...
  Firstly it comes with a concurrent model of computation (this is made precise
  by the translation to B) which allows for arbitrary interleaving of events.
  ...
  linked to the translation in B and this was heavily inspired by
...
\subsection{Modelling the loan agreement in L4}

\section{Modelling contracts in B and how it relates to L4}
...
One of these which we explored is the B language and the ProB toolset
\cite{prob_initial_paper, prob_follow_up_paper}.

In this section, we begin by providing some background on B before
...
Thereafter, we demonstrate how we can use ProB to help us
...
\inlinetodo{
  ...
  Include sections showing off the visualization that B provides, as well as
  simulating and analyzing the contract.
}
