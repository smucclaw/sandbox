...
@id = [A-Za-z][A-Za-z'_]*
@char = ($graphic # $special)
@string = [^\# \ \t \r \n]+  \" ($graphic # \")+ \" 
          | \" ($graphic # \")+ \"  [^\# \ \t \r \n]+
          | \" ($graphic # \")* \" 

 -- find complement of double quotes, comment 
 -- \" ($graphic # \")* \" 
 -- grep "^[^#\n]" 
 -- grep -n "[^aeiou]
 -- [^\n\#] + \" ($graphic # \")*  \" + [^\n]
 -- this can ignore comment lines: [^\# \ ]+ 
...
tokens :-
  ...
  @string[$special $white.]*                      { lex TokenStringLit }

  -- @string[  $white.  grep -n "[^aeiou]]*                      { lex TokenStringLit }
  --@string[ grep [^\n]]*                      { lex TokenStringLit }
