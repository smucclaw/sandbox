-- Exercises on logic:
-- - validity checker for Boolean formulas
-- - Prolog emulator
...
equiv f1 f2 = (f1 `implies` f2) `And` (f2 `implies` f1)
...
-- Attention, running the "validity" tests may take very much time!!!
runTest :: IO ()
runTest = hspec $ do
  it "nnf test" $ do
    property $ \f -> nnf (nnf f) == nnf f
  it "substAll removes all variables" $ do
    property $ \f -> \sbs -> if (map fst sbs) == fvList f then null (fvList (substAll f sbs)) else True
{-
  it "simplifyConst is equivalence transformation" $ do
    property $ \f -> valid ((simplifyConst f) `equiv` f)
  it "cnf is equivalence transformation" $ do
    property $ \f -> valid ((cnf f) `equiv` f)
-}
